# 游戏存档系统重构说明

## 📋 概述

本次重构将游戏存档系统从 **"每个存档一条记录"** 改为 **"每个用户一条记录"** 的模式。

### 旧结构 vs 新结构

| 特性 | 旧结构 | 新结构 |
|------|--------|--------|
| 记录数量 | 每个存档一条 | 每个用户一条 |
| 查询效率 | 低（需要多次查询） | 高（一次查询获取所有存档） |
| 数据关联 | 弱（难以跨存档统计） | 强（用户维度数据聚合） |
| 索引效率 | playerId + saveSlot 复合索引 | playerId 唯一索引 |

**示例对比：**

```
旧结构 - 3个存档 = 3条记录：
┌─────────┬──────────┬────────────┐
│ playerId│ saveSlot │ saveData   │
├─────────┼──────────┼────────────┤
│ user123 │    1     │ {...}      │
│ user123 │    2     │ {...}      │
│ user123 │   99     │ {...}      │
└─────────┴──────────┴────────────┘

新结构 - 3个存档 = 1条记录：
┌─────────┬─────────────────────────────┐
│ playerId│ saves (Map)                 │
├─────────┼─────────────────────────────┤
│ user123 │ {                           │
│         │   "1": { saveData },        │
│         │   "2": { saveData },        │
│         │   "99": { saveData }        │
│         │ }                           │
└─────────┴─────────────────────────────┘
```

---

## 🔧 新结构详解

### Schema 设计

```javascript
{
  playerId: String (unique),           // 用户唯一标识
  playerName: String,                  // 用户名称
  
  // 所有存档槽位（Map结构）
  saves: {
    "1": { saveData },   // 手动存档槽位1
    "2": { saveData },   // 手动存档槽位2
    ...
    "98": { saveData },  // 自动存档（固定）
    "99": { saveData }   // 快速存档（固定）
  },
  
  // 全局已读场景（跨存档）
  globalReadScenes: [1, 2, 5, 10, ...],
  
  // 全局解锁内容（跨存档）
  globalUnlockedContent: {
    cg: ["cg_01", "cg_02"],
    achievements: ["ach_01"],
    endings: ["day_ending", "night_ending"]
  },
  
  // 元数据
  metadata: {
    totalSaves: 5,              // 总存档数
    lastPlayedAt: Date,         // 最后游玩时间
    totalPlayTime: 3600,        // 总游戏时长（秒）
    gameVersion: "1.0.0"        // 游戏版本
  }
}
```

### 单个存档数据结构

```javascript
{
  saveName: "存档 1",
  description: "用户备注",
  scriptId: "chapter1-1",
  scriptName: "第一章",
  currentSceneIndex: 42,
  screenshot: "https://...",
  sceneSnapshot: {
    speaker: "角色名",
    text: "对话内容",
    bgImage: "背景图",
    timestamp: Date
  },
  progressPercentage: 65,
  gameVariables: Map<String, Any>,
  choiceHistory: [{ choice data }],
  readScenes: [1, 2, 3, 5, 10],
  unlockedContent: {
    cg: ["cg_01"],
    achievements: [],
    endings: []
  },
  playTime: 1200,
  isAutoSave: false,
  isQuickSave: false,
  isValid: true,
  savedAt: Date
}
```

---

## 🚀 数据迁移

### 运行迁移脚本

```bash
# 1. 备份数据库（强烈建议！）
mongodump --db fate_novel --out ./backup

# 2. 运行迁移脚本
cd backend
node src/scripts/migrateGameSaves.js
```

### 迁移流程

1. **备份旧表**：`game_saves` → `game_saves_old`
2. **读取旧数据**：从 `game_saves_old` 读取所有记录
3. **按用户分组**：将存档按 `playerId` 分组
4. **创建新记录**：为每个用户创建一条新记录
5. **迁移存档数据**：将所有存档槽位迁移到 `saves` Map
6. **聚合全局数据**：收集跨存档的已读场景和解锁内容

### 回滚操作

如果迁移出现问题，可以回滚到旧结构：

```javascript
// MongoDB Shell
use fate_novel;
db.game_saves.drop();                      // 删除新表
db.game_saves_old.renameCollection("game_saves");  // 恢复旧表
```

---

## 📊 API 变化

### 前端 API - 无变化！

前端 API 接口保持完全兼容，无需修改前端代码：

```javascript
// ✅ 这些 API 调用保持不变
gameSaveApi.getPlayerSaves(playerId)
gameSaveApi.getSaveBySlot(playerId, slot)
gameSaveApi.saveGame(playerId, slot, data)
gameSaveApi.quickSave(playerId, data)
gameSaveApi.autoSave(playerId, data)
```

### 后端实现 - 重构

后端 Controller 已完全重构以适配新结构：

**旧实现（多条记录）：**
```javascript
// 查询所有存档
const saves = await GameSave.find({ playerId });
```

**新实现（单条记录）：**
```javascript
// 查询用户存档记录
const playerSaves = await GameSave.findOne({ playerId });
const saves = playerSaves.getAllSaves();
```

---

## 🎯 优势

### 1. 性能提升

**查询优化：**
- 旧：查询用户所有存档需要扫描多条记录
- 新：一次查询获取用户所有数据

**示例：**
```javascript
// 旧方式 - 多次查询
const save1 = await GameSave.findOne({ playerId, saveSlot: 1 });
const save2 = await GameSave.findOne({ playerId, saveSlot: 2 });
const save99 = await GameSave.findOne({ playerId, saveSlot: 99 });

// 新方式 - 一次查询
const playerSaves = await GameSave.findOne({ playerId });
const save1 = playerSaves.getSave(1);
const save2 = playerSaves.getSave(2);
const save99 = playerSaves.getQuickSave();
```

### 2. 数据聚合

**全局数据统计：**
- 跨存档的已读场景追踪
- 全局成就和解锁内容
- 用户总游戏时长统计

**示例：**
```javascript
// 获取用户总游戏时长
const playerSaves = await GameSave.findOne({ playerId });
console.log(playerSaves.metadata.totalPlayTime); // 所有存档的总时长

// 检查用户是否看过某个场景（任意存档）
const hasReadScene = playerSaves.isSceneGlobalRead(42);
```

### 3. 存储优化

**减少记录数：**
- 1000个用户 × 5个存档/用户 = 5000条记录 → **1000条记录**
- 索引数量减少 60%+
- 数据库大小可能减少 20-30%（减少重复元数据）

---

## 📖 使用示例

### 保存游戏

```javascript
// Controller 自动处理用户记录的查找或创建
const response = await gameSaveApi.saveGame(playerId, 1, {
  scriptId: 'chapter1-1',
  currentSceneIndex: 42,
  gameVariables: { choice_5: 1 },
  choiceHistory: [...]
});

// 后端处理：
// 1. 查找或创建用户记录
// 2. 更新槽位1的存档数据
// 3. 更新全局已读场景
// 4. 保存整个用户记录
```

### 获取所有存档

```javascript
const response = await gameSaveApi.getPlayerSaves(playerId);

// 返回格式（与旧 API 兼容）：
{
  success: true,
  data: [
    { saveSlot: 1, saveName: "存档1", ... },
    { saveSlot: 2, saveName: "存档2", ... },
    { saveSlot: 98, saveName: "自动存档", isAutoSave: true },
    { saveSlot: 99, saveName: "快速存档", isQuickSave: true }
  ],
  count: 4,
  metadata: {
    totalSaves: 4,
    lastPlayedAt: "2025-12-10T...",
    totalPlayTime: 3600
  }
}
```

### 删除存档

```javascript
// 删除指定槽位
await gameSaveApi.deleteSave(playerId, 1);

// 后端处理：
// 1. 查找用户记录
// 2. 从 saves Map 中删除槽位1
// 3. 更新 metadata.totalSaves
// 4. 保存
```

---

## ⚠️ 注意事项

### 1. 数据一致性

- 迁移前务必备份数据库
- 迁移后验证数据完整性
- 保留旧表 `game_saves_old` 至少一周

### 2. 内存使用

- 每个用户记录可能包含多个存档（通常 ≤ 10个）
- 单个文档大小估算：基础数据 (~10KB) + 存档数 × 平均存档大小 (~50KB)
- 典型用户记录：10KB + 5 × 50KB = ~260KB
- MongoDB 单文档限制：16MB（远超实际需求）

### 3. 并发控制

新结构下，对同一用户的并发存档操作需要注意：

```javascript
// ❌ 错误：并发修改可能导致数据丢失
Promise.all([
  saveToSlot(playerId, 1, data1),
  saveToSlot(playerId, 2, data2)
]);

// ✅ 正确：串行操作或使用事务
await saveToSlot(playerId, 1, data1);
await saveToSlot(playerId, 2, data2);
```

---

## 🧪 测试建议

### 功能测试

- [ ] 创建新存档
- [ ] 更新现有存档
- [ ] 删除存档
- [ ] 快速存档/读档
- [ ] 自动存档
- [ ] 获取存档列表
- [ ] 跨存档数据（全局已读）

### 性能测试

```javascript
// 测试查询性能
console.time('获取用户所有存档');
const saves = await gameSaveApi.getPlayerSaves(playerId);
console.timeEnd('获取用户所有存档');

// 预期：< 50ms（本地）, < 200ms（远程）
```

---

## 📞 支持

如有问题，请检查：

1. 迁移日志输出
2. MongoDB 日志
3. 应用程序日志（controller 层）
4. 备份数据完整性

**常见问题：**

Q: 迁移后旧存档消失了？
A: 检查 `game_saves_old` 表是否存在，使用回滚命令恢复。

Q: 性能反而变慢了？
A: 确保 `playerId` 字段有唯一索引，检查查询方式是否正确。

Q: 前端报错 404？
A: 检查 API 路由是否正确，JWT Token 是否有效。

---

## 🎉 总结

本次重构带来了：

✅ **更高的查询效率**（一次查询获取所有数据）  
✅ **更好的数据聚合**（用户维度的全局统计）  
✅ **更少的数据库记录**（减少 80%+）  
✅ **完全的接口兼容**（前端无需修改）  
✅ **更强的扩展性**（便于添加用户维度功能）

迁移完成后，系统将更加高效和易于维护！🚀





