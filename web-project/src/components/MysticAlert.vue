  ...
  <Teleport to="body">
    <div v-if="modelValue" class="modal-overlay" @click.self="handleOverlayClick">
      <div class="mystic-card">
        ...
        <div class="system-notice">{{ noticeTitle }}</div>

        <div class="message-content">
          {{ message }}
          <span v-if="highlightText" class="highlight-text">{{ highlightText }}</span>
        </div>
...
.system-notice::before, .system-notice::after {
  content: '';
  display: block;
  width: 20px;
  height: 1px;
...
.btn-confirm:hover {
  ...
  letter-spacing: 5px; /* 悬停时字间距变宽，增加动感 */
